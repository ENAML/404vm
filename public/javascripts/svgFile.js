window.onload=function(){
  ...
  var colors = ['#00ff00','#2d42ff', '#fff'];
  var colors2 = ['#000','#fff','#aaa'];
  ...
  Snap.load("../images/VM_Splash02.svg", function (f) {
    ...
    function animOn() {
      if (animating) {
        ...
        facePaths.forEach(function(el) {
          el.animate({fill: colorPicker(colors)},20, animOn);
    ...
    vm404.hover(function() {
      animating = true;
      animOn();
      // circle.animate({'stroke-width': '100'},50);
      // face.animate({fill:"#0000ff"},50);
    }, function() {
        animating = false;

        facePaths.forEach(function(el) {
          el.animate({fill: colorPicker(colors2)},500);
        });

      // circle.animate({'stroke-width': '20'},50);
